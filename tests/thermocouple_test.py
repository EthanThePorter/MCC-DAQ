...
class Controller:
    ...
    @staticmethod
    def flowmeter_voltage_read(channel):
        """
        Method for reading Cole-Parmer Instrument Company Gear Pump Drive.
        Initialize channel first for analog read with Controller.initialize_analog_read()
        Function takes 3.9s on average.
        :return: Average flowmeter voltage from steady state
        """

        # Initialize array for voltage to be added to
        voltage_array = []

        # Pauses for 1 second to allow flowmeter voltage to stabilize
        time.sleep(1)

        # Reads voltage
        for _ in range(200):
            voltage = Controller.analog_read(channel)
            print(voltage)
            voltage_array.append(voltage)

        # Returns the average of the last 5 values read during steady state
        return np.average(voltage_array[25:])


channel = 8


Controller.initialize_analog_read(channel, rate=1000)
