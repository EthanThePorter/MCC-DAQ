"""
File:                       USB2416_d_bit_out.py

Library Call Demonstrated:  mcculw.ul.d_bit_out().

Purpose:                    Control each digital bit, one at a time.

Demonstration:              Output control each bit, independently
...
def run_example():
    ...
    try:
        ...
        print("Each bit will turn on until all bits are on")
        # Turn on one bit of the board in sequence...
        for bit_num in range(8):
            print("Controlling bit: {0}, writing value of 1 to the bit.".format(bit_num))
            ul.d_bit_out(board_num, DigitalPortType.FIRSTPORTA, bit_num, 1)
            sleep(1)
