"""
File:                       USB2416_d_bit_in.py

Library Call Demonstrated:  mcculw.ul.d_bit_in().

Purpose:                    Read one digital bit at a time.

Demonstration:              Monitor each digital bit in sequence.
...
def run_example():
    ...
    try:
        # Read the bit and print the value:
        for bit_num in range(8):
            bit_value = ul.d_bit_in(board_num, DigitalPortType.FIRSTPORTA, bit_num)
            print("Reading bit{0}: value: {1}.".format(bit_num, bit_value))
